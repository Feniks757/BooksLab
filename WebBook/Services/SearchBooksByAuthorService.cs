...
public class SearchBooksByAuthorService : ISearchBooksService
{
    ...
    public SearchBooksByAuthorService(IDbContextFactory<BookContext> contextFactory, IBookSearch bookSearch)
    {
    ...
    public async Task<IEnumerable<Book>> SearchBooksAsync(int userId, string searchQuery)
    {
        ...
        db.UserId = userId;
        return await _bookSearch.SearchAsync(db, searchQuery, (Book book) => book.Author);
    }
}